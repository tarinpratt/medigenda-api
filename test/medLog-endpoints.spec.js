...
describe('MedLog Endpoints', function() {
    let db 
            ...
            it(`responds w 400 when no required fields are supplied`, () => {
                const idToUpdate = 2
                    return supertest(app)
                            ...
                            .expect(400, { error: { message: `request body must contain date, time, medname, amounttaken, reason`} })
                    })
            it(`responds w 204 when updating only a subset of fields`, () => {
                const idToUpdate = 2
                const updateEntry = { medname: 'updated entry medname' }
                const expectedEntry = {
                        ...testMedLog[idToUpdate - 1],
                        ...updateEntry
                    }
